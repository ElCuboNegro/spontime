...
from challenge.model import DelayModel, ModelPersistence
...
class TestModel(unittest.TestCase):
    ...
    def test_model_data_size(self):
        # data rows should be the same as the features and target rows
        features, target = self.model.preprocess(data=self.data, target_column="delay")

        assert target.shape[0] == self.data.shape[0]
        assert features.shape[0] == self.data.shape[0]
